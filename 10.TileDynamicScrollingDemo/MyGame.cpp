...
void DrawTiles()
{
    ...
    int tilex = ScrollX / TILEWIDTH;
    int tiley = ScrollY / TILEHEIGHT;

    // calculate the number of columns and rows
    int columns = WINDOWWIDTH / TILEWIDTH;
    ...
    for (int y = 0; y <= rows; y++)
    {
        for (int x = 0; x <= columns; x++)
        {
            ...
            int tilenum = MAPDATA[((tiley + y) * MAPWIDTH + (tilex + x))];
...
void Game_Run(HWND window)
{
    ...
    // to exit
    if (KEY_DOWN(VK_ESCAPE) || controllers[0].wButtons & XINPUT_GAMEPAD_BACK)
        gameover = true;
}
