...
void player_shoot()
{
    ...
    case 5:
    {
        ...
        bullets[b3].velx = (float)(12.0 * LinearVelocityX(bullets[b3].rotation));
        bullets[b3].vely = (float)(12.0 * LinearVelocityY(bullets[b3].rotation));
        bullets[b3].x = player.x + player.width / 2;
        bullets[b3].y = player.y + player.height / 2 - bullets[b3].height / 2;
        bullets[b3].y -= 20;
        ...
        bullets[b4].velx = (float)(12.0 * LinearVelocityX(bullets[b4].rotation));
        bullets[b4].vely = (float)(12.0 * LinearVelocityY(bullets[b4].rotation));
        bullets[b4].x = player.x + player.width / 2;
        bullets[b4].y = player.y + player.height / 2 - bullets[b4].height / 2;
        bullets[b4].y += 20;
    }
    ...
    for (int n = 0; n < FRAGMENTS; n++)
    {
        if (fragments[n].alive)
        {
            ...
            if (CollisionD(player, fragments[n]))
            {
                ...
                if (fragmentx < playerx)
                    fragments[n].x += 6.0f;
                if (fragmentx > playerx)
                    fragments[n].x -= 6.0f;
                if (fragmenty < playery)
                    fragments[n].y += 6.0f;
                if (fragmenty > playery)
                    fragments[n].y -= 6.0f;
            }
...
void Draw_Background()
{
    RECT source_rect = {
        ...
        (long)scrollx + SCREENW, (long)scrolly + SCREENH};
...
void Draw_Overloading_Ship()
{
    for (int n = 0; n < 4; n++)
    {
        D3DCOLOR overcolor = D3DCOLOR_ARGB(150 + rand() % 100, 80, 255, 255);
...
void Draw_HUD()
{
    ...
    FontPrint(debugfont, 0, y - 12,
              "Core FPS = " + ToString(corefps) + " (" + ToString(1000.0 / corefps) + " ms)",
              debugcolor);
    ...
    FontPrint(debugfont, 0, y - 36,
              "Ship X,Y = " + ToString(player.x) + "," + ToString(player.y),
              debugcolor);
    ...
    FontPrint(debugfont, 0, y - 72,
              "Virtual Scroll = " + ToString(virtual_scrollx) + " / " + ToString(virtual_level_size));

    FontPrint(debugfont, 0, y - 84,
              "Fragment[0] = " + ToString(fragments[0].x) + "," + ToString(fragments[0].y));
}
